...
export class SongPlayerPage implements OnInit {
  ...
  @ViewChild("audioElem", { static: false }) audioVC: {
    nativeElement: HTMLAudioElement;
  };

  paused: boolean = false;

  person1AnimOptions: AnimationOptions = {
    path: "assets/lottie-animations/two_kids_dancing_02.json",
  };
  ...
  onAudioTimeUpdate(elem: HTMLAudioElement) {
    ...
    for (var lyric of this.currentSong.lyrics) {
      var startTimeSec: number =
        parseFloat(lyric.startTimeMinute) * 60 +
        parseFloat(lyric.startTimeSecond);
      var endTimeSec: number =
        parseFloat(lyric.endTimeMinute) * 60 + parseFloat(lyric.endTimeSecond);
      if (
        audioCurrentTimeSec >= startTimeSec &&
        audioCurrentTimeSec <= endTimeSec
      ) {
        var newLines = lyric.lyric.split("\n");
        if (
          lyric.isInstructions &&
          lyric.isInstructions.trim().toLowerCase() === "true"
        ) {
          this.hasInstructions = true;
          this.lyricLinesInstructions = newLines;
        } else if (lyric.person === "2") {
          this.hasTwoPeople = true;
          if (newLines[0] && newLines[0] !== this.lyricLinesPerson2[0]) {
            this.lyricPerson2Color = this.pickRandom([
              "lightpink",
              "pink",
              "darkred",
              "black",
          ...
          if (newLines[0] && newLines[0] !== this.lyricLinesPerson1[0]) {
            this.lyricPerson1Color = this.pickRandom([
              "skyblue",
              "cyan",
              "blue",
              "lightblue",
